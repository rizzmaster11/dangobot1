...
module.exports = class RPS extends Command {
  ...
  async run(message, args) {
    ...
    let msg = await message.channel.send({ embeds: [this.client.embedBuilder(this.client, message.author, this.client.embeds.title, this.client.language.general.rps_request.replace("<user>", user.username).replace("<author>", message.author.username), this.client.embeds.general_color)], components: [playNotPlay] })

    const choiceFilter = (i) => i.user.id == user.id;
    let firstPick, secondPick;
    await msg.awaitMessageComponent({ filter: choiceFilter, time: 60_000 }).then(async(i) => {
      await i.deferUpdate();
      if(i.customId == "rps_accept") {
        ...
        collector.on("collect", async(bttn) => {
          ...
          if(firstPick && secondPick) {
            if((firstPick == "rps_rock" && secondPick == "rps_scissors") || (firstPick == "rps_paper" && secondPick == "rps_rock") || (firstPick == "rps_scissors" && secondPick == "rps_paper")) {
              ...
              message.channel.send({ embeds: [this.client.embedBuilder(this.client, message.author, this.client.embeds.title, this.client.language.general.rps_tie.replace("<firstPlayer>", message.author).replace("<secondPlayer>", user).replace("<choice>", localeOptions[rpsOptions.indexOf(firstPick)]), this.client.embeds.general_color)] });
            }

            collector.stop("game_end");
          }
        });
        ...
        collector.on("end", async(collected, reason) => {
          ...
          if(reason == "game_end") {
            ...
            await msg.edit({ components: [choiceRow] });
          }
        ...
        await msg.edit({ components: [playNotPlay] });
  ...
  async slashRun(interaction, args) {
    ...
    let msg = await interaction.reply({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.general.rps_request.replace("<user>", user.username).replace("<author>", interaction.user.username), this.client.embeds.general_color)], components: [playNotPlay], fetchReply: true })

    const choiceFilter = (i) => i.user.id == user.id;
    let firstPick, secondPick;
    await msg.awaitMessageComponent({ filter: choiceFilter, time: 5000 }).then(async(i) => {
      if(i.customId == "rps_accept") {
        ...
        collector.on("collect", async(bttn) => {
          ...
          if(firstPick && secondPick) {
            if((firstPick == "rps_rock" && secondPick == "rps_scissors") || (firstPick == "rps_paper" && secondPick == "rps_rock") || (firstPick == "rps_scissors" && secondPick == "rps_paper")) {
              interaction.followUp({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.general.rps_won.replace("<winner>", interaction.user).replace("<winnerChoice>", localeOptions[rpsOptions.indexOf(firstPick)]).replace("<loser>", user).replace("<loserChoice>", localeOptions[rpsOptions.indexOf(secondPick)]), this.client.embeds.general_color)] });
            } else if((secondPick == "rps_rock" && firstPick == "rps_scissors") || (secondPick == "rps_paper" && firstPick == "rps_rock") || (secondPick == "rps_scissors" && firstPick == "rps_rock")) {
              interaction.followUp({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.general.rps_won.replace("<winner>", user).replace("<winnerChoice>", localeOptions[rpsOptions.indexOf(secondPick)]).replace("<loser>", interaction.user).replace("<loserChoice>", localeOptions[rpsOptions.indexOf(firstPick)]), this.client.embeds.general_color)] });
            } else if(firstPick == secondPick) {
              interaction.followUp({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.general.rps_tie.replace("<firstPlayer>", interaction.user).replace("<secondPlayer>", user).replace("<choice>", localeOptions[rpsOptions.indexOf(firstPick)]), this.client.embeds.general_color)] });
            }
        ...
        collector.on("end", async(collected, reason) => {
          ...
          if(reason == "game_end") {
            ...
            await interaction.editReply({ components: [choiceRow] });
          }
      ...
      playNotPlay.components[0].setDisabled(true);
      playNotPlay.components[1].setDisabled(true);
      await interaction.editReply({ components: [playNotPlay] })
    });
  }
};
