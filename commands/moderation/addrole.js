...
module.exports = class AddRole extends Command {
  ...
  async run(message, args) {
    ...
    message.channel.send({ embeds: [this.client.embedBuilder(this.client, message.author, this.client.embeds.title, this.client.language.moderation.role_added.replace("<user>", member.user.username).replace("<role>", role), this.client.embeds.success_color)] });
  }
  async slashRun(interaction, args) {
    ...
    member.roles.add(role).catch((err) => {});
    interaction.reply({ embeds: [this.client.embedBuilder(this.client, interaction.user, this.client.embeds.title, this.client.language.moderation.role_added.replace("<user>", member.user.username).replace("<role>", role), this.client.embeds.success_color)], ephemeral: this.client.cmdConfig.addrole.ephemeral });
  }
};
