...
module.exports = class Clear extends Command {
  ...
  async run(message, args) {
    ...
    message.channel.bulkDelete(fetched).then(async () => {
      ...
      message.channel.send({ embeds: [ this.client.embedBuilder(this.client, message.author, this.client.language.titles.error, this.client.language.moderation.messages_old, this.client.embeds.error_color)] });
    });
  
  }

  async slashRun(interaction, args) {
    let count = interaction.options.getNumber("number");
    ...
    interaction.channel.bulkDelete(fetched).then(async () => {
      interaction.reply({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.moderation.titles.clear, this.client.language.moderation.purged.replace("<count>", count), this.client.embeds.success_color)], ephemeral: this.client.cmdConfig.clear.ephemeral });
      this.client.utils.logs(this.client, interaction.guild, "Messages Purged", [{
        name: this.client.language.titles.logs.fields.user,
        desc: `${interaction.user.username}`
      }, {
        name: "Channel",
        desc: `#${interaction.channel.name}`
      }, {
        name: "Count",
        desc: count
      }], interaction.user, "PURGE");
    }).catch(error => {
      interaction.reply({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.titles.error, this.client.language.moderation.messages_old, this.client.embeds.error_color)], ephemeral: this.client.cmdConfig.clear.ephemeral });
    });
  }
};
