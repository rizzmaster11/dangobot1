...
module.exports = class Calculate extends Command {
  ...
  async run(message, args) {
    ...
    if(!args[0]) return message.channel.send({ embeds: [this.client.utils.validUsage(this.client, message, this.client.cmdConfig.calculate.usage)] });
  ...
  async slashRun(interaction, args) {
    ...
    let calcModal = new Discord.ModalBuilder()
