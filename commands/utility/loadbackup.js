...
module.exports = class LoadBackup extends Command {
  ...
  async slashRun(interaction, args) {
    await interaction.deferReply({ ephemeral: this.client.cmdConfig.loadbackup.ephemeral });
    ...
    fs.exists("./backups/" + backupId + ".json", async (exists) => {
      if(exists) {
        ...
        let m = await interaction.followUp({ embeds: [backupEmbed], components: [row], ephemeral: this.client.cmdConfig.loadbackup.ephemeral });
        ...
        collector.on("collect", async(b) => {
          await b.deferUpdate();
          if(b.customid == "load_all") {
            interaction.followUp({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.backup.title, this.client.language.backup.loading.replace("<id>", backupId), this.client.embeds.general_color)] });    
            setTimeout(async() => {
              await loadRoles(this.client, interaction, backupId);
            }, 4000);
            setTimeout(async() => {
              await loadChannels(this.client, interaction, backupId);
              collector.stop("claimed");
            }, 10000)
          } else if(b.customId == "load_roles") {
            interaction.followUp({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.backup.title, this.client.language.backup.loading.replace("<id>", backupId), this.client.embeds.general_color)] });    
            ...
            interaction.followUp({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.backup.title, this.client.language.backup.loading.replace("<id>", backupId), this.client.embeds.general_color)] });    
            setTimeout(async() => {
              await loadChannels(interaction, backupId);
              collector.stop("claimed");
            }, 4000);
          }
        });
        ...
        interaction.followUp({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.backup.title, this.client.language.backup.invalid_id.replace("<id>", backupId), this.client.embeds.error_color)], ephemeral: this.client.cmdConfig.loadbackup.ephemeral });    
      }
    });
  }
};
    ...
    return new Promise(async (resolve) => {
      ...
      setTimeout(async() => {
        await data.channelList.filter(c => c.type == Discord.ChannelType.GuildCategory).concat(data.channelList.filter(c => c.type != Discord.ChannelType.GuildCategory))
          .forEach((channel, i) => {
            message.guild.channels.create({ name: channel.name, type: channel.type }).then(ch => {
