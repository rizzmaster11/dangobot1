...
module.exports = class SaveBackup extends Command {
    ...
    const backup = {
      ...
      channelList: message.guild.channels.cache.map(ch => {
        return {
          ...
          permissions: ch.permissionOverwrites.cache.filter((c) => c.type != Discord.ChannelType.PublicThread && c.type != Discord.ChannelType.PrivateThread).map(o => {
            return {
              id: o.id,
              type: o.type,
  ...
  async slashRun(interaction, args) {
    await interaction.deferReply({ ephemeral: this.client.cmdConfig.savebackup.ephemeral });
    const bId = crypto.randomBytes(3).toString("hex");

    interaction.followUp({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.backup.title, this.client.language.backup.started, this.client.embeds.general_color)], ephemeral: this.client.cmdConfig.savebackup.ephemeral });

    const backup = {
      id: interaction.guild.id,
      channelList: interaction.guild.channels.cache.filter((c) => c.type != Discord.ChannelType.PublicThread && c.type != Discord.ChannelType.PrivateThread).map(ch => {
        return {
          name: ch.name,
          id: ch.id,
          type: ch.type,
    ...
    interaction.followUp({ embeds: [ this.client.embedBuilder(this.client, interaction.user, this.client.language.backup.title, this.client.language.backup.done.replace("<id>", bId), this.client.embeds.general_color)], ephemeral: this.client.cmdConfig.savebackup.ephemeral });  
  }
};
