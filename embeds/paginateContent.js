...
async function sliceContent(content, currentPage, perPage) {
  let page = content.slice(perPage * (currentPage - 1), perPage * currentPage);
...
module.exports = async function paginate(client, array, perPage, firstPage, message, title, color) {
  const buttonFilter = (i) => i.user.id == message.member.user.id;
  ...
  if(message.type == InteractionType.ApplicationCommand) {
    message.reply({ embeds: [embed], components: [row], fetchReply: true }).then((m) => {
      const collector = m.createMessageComponentCollector({ filter: buttonFilter, componentType: ComponentType.Button, time: 300000 });
