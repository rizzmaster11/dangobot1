...
async function submitGiveaway(client, message, data) {
  ...
  let gwConfirm = new EmbedBuilder()
    .setTitle(client.embeds.gw_confirm.title)
    .setDescription(client.embeds.gw_confirm.description.replace("<duration>", client.utils.formatTime(ms(data.duration)))
    ...
    .setColor(client.embeds.gw_confirm.color)
...
async function prizeSetup(client, message, embed, filter, data) {
  ...
  prizeCollector.on("collect", async (msg) => {
    ...
    if(!prizeArg || prizeArg.length < 3 || prizeArg.length > 32) 
      return message.channel.send({ embeds: [ client.embedBuilder(client, message.member.user, client.language.giveaway.titles.setup, client.language.giveaway.create.errors.prize, client.embeds.error_color)] });
...
async function invitesSetup(client, message, embed, filter, data) {
  ...
  invCollector.on("collect", async (msg) => {
    ...
    if(isNaN(msg.content)) 
      return message.channel.send({ embeds: [ client.embedBuilder(client, message.member.user, client.language.giveaway.titles.setup, client.language.giveaway.create.errors.invites, client.embeds.error_color)] });
...
async function messagesSetup(client, message, embed, filter, data) {
  ...
  msgCollector.on("collect", async (msg) => {
    ...
    if(isNaN(msg.content)) 
      return message.channel.send({ embeds: [ client.embedBuilder(client, message.member.user, client.language.giveaway.titles.setup, client.language.giveaway.create.errors.messages, client.embeds.error_color)] });
...
async function channelSetup(client, message, embed, filter, data) {
  ...
  channelCollector.on("collect", async (msg) => {
    ...
    if(!msg.mentions.channels.first()) 
      return message.channel.send({ embeds: [ client.embedBuilder(client, message.member.user, client.language.giveaway.titles.setup, client.language.giveaway.create.errors.channel, client.embeds.error_color)] });
...
async function durationSetup(client, message, embed, filter, inter) {
  ...
  durationCollector.on("collect", async (msg) => {
    ...
    if(isNaN(ms(msg.content)) || ms(msg.content) == 0)
      return message.channel.send({ embeds: [ client.embedBuilder(client, message.member.user, client.language.giveaway.titles.setup, client.language.giveaway.create.errors.duration, client.embeds.error_color)] });
